...
use crate::msg::{ExecuteMsg, InstantiateMsg, MigrateMsg, ModifyContractMsg, QueryMsg, Validate};
...
fn modify_contract(
    ...
    /*
     - who can modify this?
     - if executors blanked, then matching is halted. should halt be a distinct command?
     - should re-enable be a distinct command?
    */
    if !contract_info.executors.contains(&info.sender) {
        return Err(ContractError::Unauthorized);
    }
    ...
    match msg.ask_required_attributes {
        None => (),
        Some(_) => {
            let ask_required_attributes: Vec<String> = msg.ask_required_attributes.clone().unwrap();
            ...
            for ask_order in ask_orders {
                let querier = ProvenanceQuerier::new(&deps.querier);
                let none: Option<String> = None;
                let attributes_container = querier.get_attributes(ask_order.owner.clone(), none)?;
                let attributes_names: HashSet<String> = attributes_container
                    .attributes
                    .into_iter()
                    .map(|item| item.name)
                    .collect();
                if attributes_names.len() != 0 {
                    if attributes_names
                        .iter()
                        .any(|item| !ask_required_attributes.contains(item))
                    {
                        return Err(ContractError::InvalidFields {
    ...
    if bid_orders.len() > 0 {
        // can bid_fee_account ever transfer, with the same rate?
        // does the bid_fee_account owner need to approve this transfer?
        match (msg.bid_fee_rate.clone(), contract_info.bid_fee_info) {
            (None, Some(_)) => {
                return Err(ContractError::InvalidFields {
                    fields: vec!["bid_fee_rate".to_string()],
                });
            }
            (Some(_), None) => {
                return Err(ContractError::InvalidFields {
            ...
            (None, None) => {}
            (Some(bid_fee_rate), Some(bid_fee_info)) => {
                let contract_rate = bid_fee_info.rate;
                if bid_fee_rate.ne(&contract_rate) {
                    return Err(ContractError::InvalidFields {
                        fields: vec!["bid_fee_rate".to_string()],
                    });
                }
        ...
        match msg.bid_required_attributes {
            None => {}
            Some(_) => {
                let bid_required_attributes: Vec<String> =
                    msg.bid_required_attributes.clone().unwrap();

                for bid_order in bid_orders {
                    let querier = ProvenanceQuerier::new(&deps.querier);
                    let none: Option<String> = None;
                    let attributes_container =
                        querier.get_attributes(bid_order.owner.clone(), none)?;
                    let attributes_names: HashSet<String> = attributes_container
                        .attributes
                        .into_iter()
                        .map(|item| item.name)
                    ...
                    if attributes_names.len() != 0 {
                        if attributes_names
                            .iter()
                            .any(|item| !bid_required_attributes.contains(item))
                        {
                            return Err(ContractError::InvalidFields {
...
pub fn migrate(
    ...
    // can anyone migrate an existing contract?
    msg.validate()?;
...
mod tests {
    ...
    fn execute_modify_contract_valid() {
        ...
        let contract_info1 = get_contract_info(&deps.storage);

        match contract_info1 {
            Err(error) => panic!("unexpected error: {:?}", error),
            Ok(contract_info1) => {
                assert_eq!(
                    contract_info1.convertible_base_denoms,
                    vec!["con_base_1".to_string(), "con_base_2".to_string()]
                );
                assert_eq!(
                    contract_info1.executors,
                    Vec::from([Addr::unchecked("exec1"), Addr::unchecked("exec3")])
                );
                assert_eq!(
                    contract_info1.approvers,
                    Vec::from([Addr::unchecked("approver_1"), Addr::unchecked("approver_3")])
                );
                assert_eq!(
                    contract_info1.ask_fee_info,
                    Some(FeeInfo {
    ...
    #[test]
    fn execute_modify_contract_valid_ask_attributes() {
        let mut deps = mock_dependencies(&[]);
        ...
        let asker_info1: MessageInfo = mock_info("asker1", &[coin(100, "base_denom")]);
        let create_ask_msg1 = ExecuteMsg::CreateAsk {
            base: "base_denom".into(),
            id: "ab5f5a62-f6fc-46d1-aa84-51ccc51ec367".into(),
            price: "2".into(),
            quote: "quote_1".into(),
        ...
        // querier can only query last loaded entry, wipes away older query entries
        deps.querier.with_attributes(
            "asker2",
            &[
                ("ask_tag_1", "ask_tag_1_value", "String"),
        ...
        let asker_info2 = mock_info("asker2", &[coin(100, "base_denom")]);
        let create_ask_msg2 = ExecuteMsg::CreateAsk {
            base: "base_denom".into(),
            id: "ab5f5a62-f6fc-46d1-aa84-51ccc51ec368".into(),
            price: "3".into(),
            quote: "quote_2".into(),
        ...
        match contract_info1 {
            ...
            Ok(contract_info1) => {
                assert_eq!(
                    contract_info1.executors,
                    Vec::from([Addr::unchecked("exec_1"), Addr::unchecked("exec_3")])
                );
                assert_eq!(
                    contract_info1.convertible_base_denoms,
                    vec!["con_base_1".to_string(), "con_base_2".to_string()]
                );
                let fee_info: Option<FeeInfo> = Some(FeeInfo {
                    account: Addr::unchecked("fee_acct_1"),
                    rate: "0.123".to_string(),
                });
                assert_eq!(contract_info1.ask_fee_info, fee_info);

                assert_eq!(
                    contract_info1.ask_required_attributes,
                    vec![
                        "ask_tag_1".to_string(),
                        "ask_tag_2".to_string(),
                        "ask_tag_3".to_string(),
    ...
    #[test]
    fn execute_modify_contract_valid_bid_attributes() {
        let mut deps = mock_dependencies(&[]);
        ...
        let bidder_info: MessageInfo = mock_info("bidder1", &[coin(200, "quote_1")]);
        let create_bid_msg = ExecuteMsg::CreateBid {
            id: "ab5f5a62-f6fc-46d1-aa84-51ccc51ec369".into(),
            base: "base_denom".into(),
            fee: None,
            price: "2".into(),
            quote: "quote_1".into(),
        ...
        match modify_contract_response {
            Ok(_) => {}
            Err(_) => panic!("expected modifyContract validation to fail"),
        }
        ...
        match contract_info {
            ...
            Ok(contract_info1) => {
                ...
                assert_eq!(contract_info1.bid_fee_info, None);

                assert_eq!(
                    contract_info1.bid_required_attributes,
                    vec![
                        "bid_tag_1".to_string(),
                        "bid_tag_2".to_string(),
                        "bid_tag_3".to_string(),
    ...
    fn execute_modify_contract_invalid_attributes_conflict() {
        ...
        let asker_info: MessageInfo = mock_info("asker1", &[coin(100, "base_denom")]);
        let create_ask_msg1 = ExecuteMsg::CreateAsk {
            base: "base_denom".into(),
            id: "ab5f5a62-f6fc-46d1-aa84-51ccc51ec367".into(),
            price: "2".into(),
        ...
        let asker_info = mock_info("asker2", &[coin(100, "base_denom")]);
        let create_ask_msg2 = ExecuteMsg::CreateAsk {
            base: "base_denom".into(),
            id: "ab5f5a62-f6fc-46d1-aa84-51ccc51ec368".into(),
            price: "3".into(),
    ...
    #[test]
    fn execute_modify_contract_transfer_executors() {
        let mut deps = mock_dependencies(&[]);
        ...
        let modify_contract_msg = ExecuteMsg::ModifyContract {
            approvers: Some(vec!["approver_1".into(), "approver_3".into()]),
            executors: Some(vec!["exec_3".into()]),
            ask_fee_rate: Some("0.123".into()),
            ask_fee_account: Some("fee_acct_1".into()),
            bid_fee_rate: None,
            bid_fee_account: None,
        ...
        let exec_info = mock_info("exec_3", &[]);
        let modify_contract_msg = ExecuteMsg::ModifyContract {
            approvers: Some(vec!["approver_1".into(), "approver_3".into()]),
            executors: Some(vec!["exec_1".into()]),
            ask_fee_rate: Some("0.123".into()),
            ask_fee_account: Some("fee_acct_1".into()),
            bid_fee_rate: None,
            bid_fee_account: None,
    ...
    #[test]
    fn execute_modify_contract_valid_attributes() {
        let mut deps = mock_dependencies(&[]);
    ...
    #[test]
    fn execute_modify_contract_valid_ask_fee() {
        let mut deps = mock_dependencies(&[]);
    ...
    fn execute_modify_contract_invalid_conflicting_bid_attributes() {
        ...
        let bidder_info = mock_info("bidder1", &[coin(200, "quote_1")]);
        let create_bid_msg = ExecuteMsg::CreateBid {
    ...
    #[test]
    fn execute_modify_contract_valid_non_conflicting_bid_attributes() {
        let mut deps = mock_dependencies(&[]);
